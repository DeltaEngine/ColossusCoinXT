...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                // Fill vin
                //
                // Note how the sequence number is set to max()-1 so that the nLockTime set above actually works.
                const uint32_t nSequenceFinal = (txNew.nLockTime != 0) ? (CTxIn::SEQUENCE_FINAL - 1) : CTxIn::SEQUENCE_FINAL;
                BOOST_FOREACH (const PAIRTYPE(const CWalletTx*, unsigned int) & coin, setCoins)
                    txNew.vin.push_back(CTxIn(coin.first->GetHash(), coin.second, CScript(), nSequenceFinal));
...
void CWallet::AutoZeromint()
{
    ...
    // zCOLX are integers > 0, so we can't mint 10% of 9 COLX
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zCOLX\n", nBalance);
        return;
    }

    // Percentage of zCOLX we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zCOLX (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zCOLX amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zCOLX amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // ZCDENOMINATIONS: fix this
    // Use the biggest denomination smaller than the needed zCOLX We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough COLX to mint this denomination
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    LogPrintf("zc minting: in/min: %s - %s\n", FormatMoney(nValue), FormatMoney(libzerocoin::CoinDenomination::ZQ_MIN * COIN));
    ...
    while (true) {
        ...
        LogPrintf("zc minting: remaining/fee: %s, %s\n", FormatMoney(nValueRemaining), FormatMoney(nFeeRet));
        ...
        LogPrintf("zc minting tx-out/total: %s - %s\n", FormatMoney(nValueNewMint), FormatMoney(nMintingValue));
    }
    ...
    if (isZCSpendChange) {
        ...
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            ...
            CAmount nValueSelected = coin.first->vout[coin.second].nValue;
            LogPrintf("zc minting tx-in (selected) / in: %s - %s\n", FormatMoney(nValueSelected), FormatMoney(nValueIn));
        }
    }

    LogPrintf("zc minting fees/total/in/feeRet: %s, %s, %s, %s\n", FormatMoney(nFee), FormatMoney(nTotalValue), FormatMoney(nValueIn), FormatMoney(nFeeRet));

    //any change that is less than 'dust' limit will be ignored and given as an extra fee
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to COLX
    CAmount nChange = nValueIn - nTotalValue; // Fee already accounted for in nTotalValue
    if (nChange > 0 && !isZCSpendChange) {
        ...
        if (!outChange.IsDust(::minRelayTxFee)) {
            txNew.vout.push_back(outChange);
            LogPrintf("zc minting tx-out (change/to self) / in: %s - %s\n", FormatMoney(nChange), FormatMoney(nValueIn));
        } else {
            LogPrintf("zc minting dust / in: %s - %s\n", FormatMoney(nChange), FormatMoney(nValueIn));
        }
    } else {
        if (reservekey)
            reservekey->ReturnKey();
    ...
    // Sign if these are COLX outputs - NOTE that zCOLX outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    try {
        ...
        //Add the coin spend into a COLX transaction
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                //Tried to spend an already spent zCOLX
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the the smallest available denomination (= 1) of zCOLX"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCOLX transaction
    if (vSelectedMints.empty()) {
        ...
        // Select the zCOLX mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to COLX address to the transaction (the actual primary spend taking place)
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zCOLX was successful
